...
function limpiarLocalStorage() {
  localStorage.clear();
  alert('LocalStorage ha sido limpiado completamente.');
}
...
function guardarFecha(maquina, mesesMantenimiento) {
  ...
  alert('Fecha y Usuario guardadas para la Máquina ' + maquina);
}

function guardarCaracteristicas(maquina) {
  ['usuario'].forEach(attr => {
    const input = document.getElementById(attr + maquina);
    if (input) { // Asegurarse de que el elemento existe antes de intentar usarlo
